...
构建产物**未做代码签名**，首次启动系统会拦截：

- **macOS** —— 提示来自身份不明的开发者。右键点击应用选择「打开」，或清除隔离属性：
...
- **没有代码签名与公证**，因此有上面提到的首次启动拦截。
- **没有自动更新。** 菜单 Help → Desktop Releases 会打开发布页。
- **上游仍是开发者预览版。** 破坏性变更频繁，升级 `dshVersion` 后建议先做一次冒烟测试。
